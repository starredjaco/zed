...
use util::{ResultExt as _, debug_panic, paths::PathMatcher};
use workspace::{
...
impl ReusedResults {
    ...
    fn prune_stale_paths_before(
        ...
        {
            excerpts.remove_excerpts(stale_path, cx);
                ...
                let batch_changed = project_search.excerpts.update(cx, |excerpts, cx| {
                    ...
                    for (path_key, buffer, buffer_snapshot, excerpt_ranges, ranges) in batch {
                        if reused_results
                            .last_seen_path
                            .as_ref()
                            .is_some_and(|last| *last >= path_key)
                        {
                            debug_panic!(
                        ...
                        batch_changed |=
                            reused_results.prune_stale_paths_before(Some(&path_key), excerpts, cx);
                        let previous_span = reused_results.previous_ranges_for(&path_key);
                        ...
                        applied.push((previous_span, ranges));
                    }
                    let snapshot = excerpts.snapshot(cx);
                    for (previous_span, ranges) in applied {
