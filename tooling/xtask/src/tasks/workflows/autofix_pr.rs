...
fn run_autofix(pr_number: &WorkflowInput, run_clippy: &WorkflowInput) -> NamedJob {
    ...
    named::job(use_clang(
        Job::default()
            ...
            .add_step(install_cargo_shear().if_condition(Expression::new(run_clippy.to_string())))
            .add_step(run_cargo_fix().if_condition(Expression::new(run_clippy.to_string())))
            .add_step(run_cargo_shear_fix().if_condition(Expression::new(run_clippy.to_string())))
